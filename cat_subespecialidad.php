<?php 
  ...
  if(!array_key_exists(MOD_SUBESPECIALIDAD, $perm['children'])){
    ...
    $perm = $perm['children'][MOD_SUBESPECIALIDAD];
    $perm = $perm['action'];
  }
        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Subespecialidades <small>Catálogo de Subespecialidades</small></h2>
                ...
                <div class="modal-body">
                  <form class="form-horizontal form-label-left">
                    <div class="form-group">
                      <label for="item-desc" class="control-label col-md-3 col-sm-3 col-xs-12">Especialidad</label>
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        <select name="item-esp" id="item-esp" class="form-control col-md-7 col-xs-12">
                          <?php 
                            $SQLe = "SELECT ID, descripcion FROM pre_especialidad WHERE status = 1 ORDER BY descripcion; ";
                            $rese = mysql_query($SQLe);

                            while ($esp = mysql_fetch_assoc($rese)) {
                              echo '<option value="'.$esp['ID'].'">'.utf8_encode($esp['descripcion']).'</option>';
                            }
                          ?>
                        </select>
                      </div>
                    ...
                    <div class="form-group">
                      <label class="control-label col-md-3 col-sm-3 col-xs-12" for="item-nom">Subespecialidad <span class="required">*</span>
                      </label>
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        <input type="text" id="item-nom" required="required" class="form-control col-md-7 col-xs-12" placeholder="Nombre de la subespecialidad">
                      </div>
                    </div>
                    <!--div class="form-group">
              ...
              <div class="modal-dialog">
                  <div class="modal-content">
                      <div class="modal-header">
                          ...
                          <h4 class="modal-title" id="myModalLabel">Eliminar Subespecialidad</h4>
                      </div>
                      <div class="modal-body">
                          <p>¿Estas seguro de eliminar la subespecialidad '<strong></strong>'?</p>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('body').on('click', '.btn-edit', function(){
            id = $(this).parent().data('id');
            esp = $(this).parent().data('esp');
            nom = $(this).parent().prev().html();
            //desc = $(this).parent().prev().html();
            $('#frm-item .modal-title').html('Editar Especialidad');
            $('#item-esp').val(esp);
            $('#item-nom').val(nom);
        ...
        $('#btnSave').click(function() {
          data = {
              ...
              esp: $.trim($('#item-esp').val()), 
              esps: $('#item-esp option:selected').text()
              };
          ...
          if(data.nom.length < 3){
            ...
            act = 'addSubesp';
            if(parseInt(data.id,10) > 0) act = 'editSubesp';
            NProgress.start();
        ...
        $('#btnDelete').click(function() {
            ...
            nom = trp.find('td:eq(1)').html();
            btn = $(this);
            btn.addClass('disabled');
            NProgress.start();
            $.post(
                '/core/precargas.php',
                {action: 'delSubesp', nom: nom, id: id},
                function(resp){
                  if(resp.res){
                    $('#tbl-items').DataTable().destroy();
                    trp.remove();
      ...
      function addItemRow(item, act){
        ...
        tr.append('<td>'+item.esp+'</td>');
        tr.append('<td>'+item.nombre+'</td>');
        tr.append('<td data-id="'+item.ID+'" data-esp="'+item.id_especialidad+'" class="text-center">'+act+'</td>');
