<?php 
  ...
  if(!array_key_exists(MOD_PUESTOS, $perm['children'])){
    ...
    $perm = $perm['children'][MOD_PUESTOS];
    $perm = $perm['action'];
  }
        ...
        <div class="right_col" role="main">
          ...
          <div class="row">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Puestos <small>Catálogo de puestos</small></h2>
                  ...
                  <form class="form-horizontal form-label-left">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-9 col-sm-9 col-xs-12">
                        <input type="text" id="item-nom" required="required" class="form-control col-md-7 col-xs-12" placeholder="Nombre del Puesto">
                      </div>
                    </div>
                    <div class="form-group">
          ...
          <div class="modal fade" id="frm-item-del" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                  <div class="modal-content">
                      ...
                      <div class="modal-body">
                          <p>¿Estas seguro de eliminar el Puesto '<strong></strong>'?</p>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#btnAdd').click(function(event) {
          $('#frm-item .modal-title').html('Agregar Puesto');
          $('#frm-item input').val('');
          $('#item-id').val(0);
          $('#frm-item').hide();
        });
        ...
        $('body').on('click', '.btn-edit', function(){
            ...
            $('#frm-item .modal-title').html('Editar Puesto');
            $('#item-nom').val(nom);
            $('#item-desc').val(desc);
        ...
        $('#btnSave').click(function() {
          ...
          if(data.nom.length < 3){
            ...
            act = 'addPuesto';
            if(parseInt(data.id,10) > 0) act = 'editPuesto';
            NProgress.start();
        ...
        $('#btnDelete').click(function() {
            ...
            $.post(
                'core/precargas.php',
                {action: 'delPuesto', nom: nom, id: id},
                function(resp){
                  if(resp.res){
                    $('#tbl-items').DataTable().destroy();
                    trp.remove();
      ...
      function addItemRow(item, act){
        nuevo = true;
        tr = $('<tr></tr>');
        if($("#tbl-items [data-id='"+item.id_puesto+"']").length > 0){
          nuevo = false;
          tr = $("#tbl-items [data-id='"+item.id_puesto+"']").parents('tr');
          tr.empty();
        }

        tr.append('<td>'+item.nombre+'</td>');
        tr.append('<td>'+item.descripcion+'</td>');
        tr.append('<td data-id="'+item.id_puesto+'" class="text-center">'+act+'</td>');
