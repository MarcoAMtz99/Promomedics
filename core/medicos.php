...
switch ($action) {
	...
	case 'saveDigitales':
		if($edita){
			...
			if($res){
				//$detalle = $cons;
				//$log->setDatos('Guarda Médico Med Dig'.$detalle,$detalle,$cons,MEDICOS);
            	//$log->saveLog();
	...
	case 'getRedes':
		...
		$actions = '';
		if($edita){
			$actions = '<button type="button" class="btn btn-xs btn-primary btnred-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
			$actions .= '<button type="button" class="btn btn-xs btn-danger btnred-del" title="Eliminar"><i class="fa fa-trash"></i></button>';
		}
	...
	case 'addRed':
		if($edita){
			...
			if($id > 0){
				$detalle = $id.' '.$nom;
				$log->setDatos('Alta Red Social Médico'.$detalle,$dataString,$id,MEDICOS);
            	$log->saveLog();

            	$item = array('ID' => $id, 'nombre' => $data->nom, 'link' => $data->link);
            	$actions = '<button type="button" class="btn btn-xs btn-primary btnred-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
				$actions .= '<button type="button" class="btn btn-xs btn-danger btnred-del" title="Eliminar"><i class="fa fa-trash"></i></button>';
	...
	case 'addStaff':
		if($edita){
			...
			if($id > 0){
            	...
            	$item = array('ID' => $id, 'nombre' => $data->nom.' '.$data->ape, 'especialidad' => $data->esps, 'email' => $data->mail, 'rol' => $data->rol, 'cedula_medico' => $data->ced);
            	$actions = '<button type="button" class="btn btn-xs btn-primary btnstaff-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
				$actions .= '<button type="button" class="btn btn-xs btn-danger btnstaff-del" title="Eliminar"><i class="fa fa-trash"></i></button>';
	...
	case 'getListasDesc':
		...
		$actions = '';
		if($edita){
			$actions = '<button type="button" class="btn btn-xs btn-primary btndescu-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
			$actions .= '<button type="button" class="btn btn-xs btn-danger btndescu-del" title="Eliminar"><i class="fa fa-trash"></i></button>';
		}
	...
	case 'addListaDesc':
		if($edita){
			...
			if($id > 0){
				$detalle = $id.' '.$nom;
				$log->setDatos('Alta Lista Desc Médico'.$detalle,$dataString,$id,MEDICOS);
            	$log->saveLog();

            	$item = array('ID' => $id, 'motivo' => $data->motivo, 'descuento' => $data->descu);
            	$actions = '<button type="button" class="btn btn-xs btn-primary btndescu-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
				$actions .= '<button type="button" class="btn btn-xs btn-danger btndescu-del" title="Eliminar"><i class="fa fa-trash"></i></button>';
	...
	case 'getDescServ':
		...
		echo json_encode(array('descuento' => $default, 'items' => $items));
		break;
	...
	case 'getPromos':
		...
		$SQL = "SELECT p.ID, p.nombre, s.nombre AS servicio, s.costo, p.costo_desc, 
		...
		$actions = '';
		if($edita){
			$actions = '<button type="button" class="btn btn-xs btn-primary btnpromo-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
			$actions .= '<button type="button" class="btn btn-xs btn-danger btnpromo-del" title="Eliminar"><i class="fa fa-trash"></i></button>';
		}
	...
	case 'getPromo':
		...
		$SQL = "SELECT p.ID, p.nombre, p.clave, s.nombre AS servicio, p.fk_servicio, s.descripcion, s.costo, 
	...
	case 'addPromo':
		if($edita){
			$med = $data->med;
			$serv = intval($data->servid);
			$ini = date2DB($data->desde);
			...
			if(mysqli_num_rows($resp) == 0){
				...
				if($id > 0){
					$detalle = $id.' '.$nom;
					$log->setDatos('Alta Promoción Médico'.$detalle,$dataString,$id,MEDICOS);
	            	$log->saveLog();

	            	$item = array('ID' => $id, 'nombre' => $data->nom, 'servicio' => $data->serv, 'costo' => $data->costo, 'costo_desc' => $data->costod, 
	            					'vigencia' => $data->desde.' - '.$data->hasta, 'calificacion' => 0);
	            	$actions = '<button type="button" class="btn btn-xs btn-primary btnpromo-edit" title="Editar"><i class="fa fa-pencil"></i></button>';
					$actions .= '<button type="button" class="btn btn-xs btn-danger btnpromo-del" title="Eliminar"><i class="fa fa-trash"></i></button>';

					$arrRes = array('error' => false, 'item' => $item, 'act' => $actions);	
				}else{
					$arrRes = array('error' => true, 'elem' => 'gral');
				}
			}else{
				$arrRes = array('error' => true, 'elem' => 'cve');
			}
		}else{
			$arrRes = array('error' => true, 'elem' => 'gral', 'msg' => 'Acceso Restringido.');
		}
	...
	case 'califPromo':
		if($edita){
			...
			if($res){
				...
				$arrRes = array('error' => true, 'elem' => 'gral', 'msg' => 'Ocurrio algo extraño');
			}
		}else{
			$arrRes = array('error' => true, 'elem' => 'gral', 'msg' => 'Acceso Restringido.');
		}
